...
| SEC-2 | **GitHub PAT is broad and non-expiring** — the fine-grained token `CludeUseToken` used to push to `haydermaala/Labs-MidWare` has no expiration date and was granted a wide set of repository permissions (≈35 write scopes) plus access to another repo (`CRM`). A leaked long-lived token is a standing compromise risk, and the broad scope widens blast radius. | Security | Medium | High | Set an expiration and rotate on a schedule; trim to least privilege (Contents + Administration + Metadata for this repo only); consider a dedicated CI/deploy token separate from the developer's personal token; never store the token in the repo or logs. | OPEN |
| SEC-3 | **Branch protection + native secret scanning unavailable on the current plan** — `main` protection and GitHub secret scanning require GitHub Pro (or a public repo) for private repositories, so unreviewed direct pushes to `main` are currently possible. | Security / Governance | Medium | Medium | Interim: enforce review discipline via PRs by convention; the CI `gitleaks` job provides secret scanning independent of plan. To enforce: upgrade to GitHub Pro/Team (enables branch protection + secret scanning + push protection) or use an organization. Re-run the protection setup once available. | OPEN |
