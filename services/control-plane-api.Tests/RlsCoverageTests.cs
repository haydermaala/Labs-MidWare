...
public sealed class RlsCoverageTests
{
    ...
    private static readonly HashSet<string> GlobalTables = new()
    {
        ...
        // The P2 permission catalog mirror (PermissionCatalogSync) — code is
    ...
    // Coverage spans both RLS migrations: P1's tenant tables + the P3 scope/role/
    // approval tables (AddP3RowLevelSecurity).
    private static HashSet<string> CoveredTables() =>
        AddRowLevelSecurity.Policies.Select(p => p.Table)
            .Concat(AddP3RowLevelSecurity.Policies.Select(p => p.Table))
            .ToHashSet();
    ...
    public void Every_Rls_Policy_Targets_A_Real_Non_Global_Table()
    {
        ...
        var allPolicies = AddRowLevelSecurity.Policies.Concat(AddP3RowLevelSecurity.Policies);
        foreach (var (table, _) in allPolicies)
        {
    ...
    public void All_Ten_Known_Tenant_Tables_Are_Covered()
    {
        ...
        var covered = AddRowLevelSecurity.Policies.Select(p => p.Table).ToHashSet();
        string[] expected =
        {
    ...
    [Fact]
    public void All_Six_P3_Tenant_Tables_Are_Covered()
    {
        // Explicit lock on the P3 coverage set (AddP3RowLevelSecurity).
        var covered = AddP3RowLevelSecurity.Policies.Select(p => p.Table).ToHashSet();
        string[] expected =
        {
            "scopes", "role_assignments", "sod_rules", "custom_roles",
            "role_permissions", "approval_requests",
        };
        foreach (var table in expected)
        {
            Assert.True(covered.Contains(table), $"expected RLS coverage for '{table}'");
